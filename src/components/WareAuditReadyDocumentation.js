const docs = [
  {
    icon: (
      <svg viewBox="0 0 24 24" fill="none" className="w-5 h-5 text-emerald-700 shrink-0 mt-0.5" stroke="currentColor" strokeWidth={1.8}>
    ...
    <section className="bg-[#f0f4ee] py-20 px-6">
      <div className="max-w-5xl mx-auto">
        ...
        <div className="text-center mb-12">
          <h2 className="text-3xl lg:text-4xl font-extrabold text-emerald-900 tracking-tight mb-5">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 border-l border-t border-slate-200 rounded-2xl overflow-hidden">
          {docs.map((doc) => (
            <div
              key={doc.title}
              className="flex gap-4 items-start p-7 border-r border-b border-slate-200 bg-white hover:bg-emerald-50/40 transition-colors duration-200"
            >
              <div className="w-9 h-9 rounded-xl bg-emerald-50 border border-emerald-100 flex items-center justify-center shrink-0">
                {doc.icon}
              </div>
              <div>
                <p className="text-[13px] font-extrabold text-emerald-900 mb-1.5">{doc.title}</p>
